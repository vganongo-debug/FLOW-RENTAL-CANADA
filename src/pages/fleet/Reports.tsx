...
const UTILISATION_BY_VEHICLE = VEHICLES.slice(0, 10).map((v, i) => {
  ...
  return {
    ...
    fill: v.owner === 'flow' ? '#015FC4' : '#B30307',
  }
})
...
const TIER_REV = [
  { tier: 'Flow GO',       revenue:  6_120, fill: '#347FCC' },
  { tier: 'Flow Drive',    revenue: 14_440, fill: '#015FC4' },
  { tier: 'Flow Terrain',  revenue: 18_220, fill: '#7D0205' },
  { tier: 'Flow Prestige', revenue: 23_180, fill: '#B30307' },
  { tier: 'Flow Elite',    revenue: 29_640, fill: '#081D38' },
]
      ...
      <ReportShell title="Utilisation by vehicle" subtitle="Days on rent / 28 days · teal = Flow, copper = Partner">
        <ResponsiveContainer width="100%" height={360}>
          <BarChart data={UTILISATION_BY_VEHICLE} layout="vertical" margin={{ left: 30 }}>
            ...
            <XAxis type="number" tick={{ fontSize: 11, fill: '#4A5C74' }} unit="%" />
            <YAxis dataKey="label" type="category" tick={{ fontSize: 11, fill: '#4A5C74' }} width={100} />
            <Tooltip />
      ...
      <ReportShell title="Utilisation trend · last 30 days" subtitle="Fleet-wide daily utilisation %">
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={UTILISATION_TREND}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#4A5C74' }} unit="%" />
            <Tooltip />
            <Line type="monotone" dataKey="utilisation" stroke="#015FC4" strokeWidth={2.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </ReportShell>
    </>
      ...
      <ReportShell title="Add-ons revenue" subtitle="Last 30 days · including attach rate">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={ADDONS}>
            <CartesianGrid stroke="#E6EFF9" vertical={false} />
            <XAxis dataKey="addon" tick={{ fontSize: 11, fill: '#4A5C74' }} />
            <YAxis tick={{ fontSize: 11, fill: '#4A5C74' }} />
            <Tooltip />
            <Bar dataKey="revenue" fill="#B30307" radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </ReportShell>
    </>
